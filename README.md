...
```json
{
  "mcpServers": {
    "ada-mcp": {
      "command": "pnpm",
      "args": ["dlx", "@ada-mcp/launcher@0.1.53", "--install-deps=playwright"]
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "ada-mcp": {
      "command": "npx",
      "args": ["-y", "@ada-mcp/launcher@0.1.53", "--install-deps=playwright"]
    }
  }
}
```
...
- npm 发布版本以 `@ada-mcp/launcher` / `@ada-mcp/mcp-server` 的 `package.json` 为准（当前示例 **`0.1.53`**，发布前请 `npm run mcp:check:versions` 核对）
