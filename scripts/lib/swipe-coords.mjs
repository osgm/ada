/** 滑动坐标解析（与 packages/driver-rpc/src/swipe-coords.ts 同步） */
...
function presetKey(name) {
  ...
  const t = name.trim();
  if (SWIPE_POINT_PRESETS[t]) return t;
  return Object.keys(SWIPE_POINT_PRESETS).find((k) => k.toLowerCase() === t.toLowerCase());
}

function asPair(point) {
  if (typeof point === "string") {
    const key = presetKey(point);
    if (!key) throw new Error(`swipe: 未知占位符 "${point}"`);
...
function resolveAxis(value, dim, relative) {
  ...
  if (typeof value !== "string") throw new Error(`swipe: 无效坐标分量 ${value}`);
...
export function resolveSwipePoint(point, screen, options = {}) {
  ...
  return [resolveAxis(xVal, screen.width, relative), resolveAxis(yVal, screen.height, relative)];
}
...
/** @deprecated 驱动层兼容；fluent 已解析为像素时 relative 应为 false */
export function normalizedSwipePoints(screen, from, to, options = {}) {
  if (options.relative === true) {
    const norm = (p) => [Math.round(p[0] * screen.width), Math.round(p[1] * screen.height)];
