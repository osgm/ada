...
async function bundleRuntimePlugins() {
  ...
  await build({
    entryPoints: [path.join(root, "plugins", "driver-appium", "src", "index.ts")],
    outfile: path.join(pluginBuildDir, "driver-appium.cjs"),
    bundle: true,
    platform: "node",
    format: "cjs",
    target: "node18",
...
async function verifyReleaseArtifacts() {
  ...
  await run("node", [path.join(root, "scripts", "verify-entrypoints.mjs")]);
}
...
async function main() {
  await prepareDirs();
  await run("node", [path.join(root, "scripts", "generate-bundled-config.mjs")]);
  await bundleAgent();
  await bundleWeb();
