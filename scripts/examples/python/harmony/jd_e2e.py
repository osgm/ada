#!/usr/bin/env python3
"""京东鸿蒙示例 — 与 jd-e2e.mjs 同 10 步

运行（仓库根目录）：python scripts/examples/python/harmony/jd_e2e.py
或：npm run test:jd-harmony:py
"""
import sys
...
from ada_client import device, dir, step_log, open, init, exit, wait
...
def main() -> None:
    ...
    phone = open(device(type="harmony", session_id="jd-harmony"))
    ...
    try:
        ...
        step_log("[3] swipe start")
        phone.swipe(SWIPE_RIGHT_FROM, SWIPE_RIGHT_TO, SWIPE_MS, relative=True, times=3)
        phone.swipe(SWIPE_LEFT_FROM, SWIPE_LEFT_TO, SWIPE_MS, relative=True, times=2)
        step_log("[3] swipe done")

        print("[4] 上滑 2 次，下滑 2 次")
        step_log("[4] swipe vertical start")
        phone.swipe(SWIPE_UP_FROM, SWIPE_UP_TO, SWIPE_MS, relative=True, times=2)
        phone.swipe(SWIPE_DOWN_FROM, SWIPE_DOWN_TO, SWIPE_MS, relative=True, times=2)
