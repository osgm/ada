...
| 方法 | 参数 | 说明 |
|------|------|------|
| `goto(url, opts?)` | `url` + 可选 `{ waitUntil, navigationTimeoutMs }` | **推荐**：当前标签打开；默认 `waitUntil: domcontentloaded` |
| `tab(url)` | `url: string` | 同 `goto` |
| `find(loc)` | 见 [定位](#定位-by--find) | 查找元素，返回句柄 |
| `locator(loc)` | 同 `find` | 别名 |
| `keyboard.press(key)` | `key: string` | 如 `"Enter"` |
| `screenshot(path)` | 文件路径；可选 `{ fullPage: true }` | 截图（**默认视口**，更快） |
...
本地与 MCP 均优先走驱动内 **`custom.action=dismissPopups`**（单次往返）；旧 bundled 驱动无此 action 时自动回退多轮 `custom.evaluate` / `click`。
...
| 项 | 行为 | 说明 |
|----|------|------|
| `navigate` / `goto` | 默认 `waitUntil: domcontentloaded` | 重站（如京东）`load` 事件可能超时；需完整 load 时传 `{ waitUntil: "load" }` |
| `dismissPopups` | 驱动内一次执行 | 减少本地桥接的多次 `custom` 往返 |
| `screenshot` | 默认视口 | 全页传 `{ fullPage: true }` |
| 四场景示例 | 每场景独立 `open`/`close` | **教学用**；生产脚本应复用同一会话，避免重复冷启动 |

```javascript
// 重站可选：显式放宽导航超时
await page.goto(HOME_URL, { navigationTimeoutMs: 45_000 });

// 需要整页长截图时
await page.screenshot(`${OUT}/full.png`, { fullPage: true });
```

---

## 移动设备对象方法（Android / 鸿蒙 / iOS）

由 `open(device(...))` 返回，记为 `phone`。三平台大部分方法一致；**多参数方法**差异见下表。

| 方法 | Android | 鸿蒙 | iOS | 说明 |
|------|---------|------|-----|------|
| `wake()` | adb `KEYCODE_WAKEUP` | shell `power-shell wakeup` | `deviceAdmin.wake` | 唤醒屏幕 |
| `killAllApps(opts?)` | `ps` → `kill` | `aa force-stop` | `deviceAdmin.killAllApps` | 可选 `excludePackages` |
| `swipe(from, to, opts?)` | 见下表 | 见下表 | 见下表 | 起点→终点滑动，时长/次数可配 |
| `goto(target, ...)` | `(appId, settleMs?)` | `(appId, abilityId?, settleMs?)` | `(bundleId, settleMs?)` | 包名启动；文案 → `find`+`click` |
| `back(times?, gapMs?)` | 系统返回 | 系统返回 | WDA 多级 fallback | 默认 1 次 |
| `dismissPopups(arg?)` | 驱动 fast-path | 同左 | 同左（单次 dump） | 失败回退 legacy 多轮 click |
| `fillSearch(text, hints?)` | 见下表 | 见下表 | 见下表 | 启发式搜索（recipe） |
| `pressHome()` | — | — | — | 系统 Home 键 |
| `find(loc)` / `locator(loc)` | 见 [定位](#定位-by--find) | 同左 | 同左 | 查找元素 |
| `screenshot(path)` | 文件路径 | 同左 | 同左 | 截图 |
| `exit(appId)` | 包名 | 包名 | bundleId | 结束指定 App |
| `close(opts?)` | 关会话 | 同左 | 同左 | 默认 `exit()`；`keepApp: true` 仅关会话 |

**`goto` 启动后等待（settle）**：Android / 鸿蒙默认 `launch_settled`（UI 节点数稳定）；**iOS 默认 `timeout`**（固定毫秒），避免 WDA `/source` 反复 dump 超时。可用 `ADA_WAIT_UNTIL` 或 `goto({ wait: { until: ... } })` 覆盖。

**鸿蒙独有**：`suspend()`、`type(text)`（无 locator 向焦点输入）。
...
| `target` | 行为 |
|----------|------|
| 包名 / bundleId `com.example.app` | `launchApp` 启动应用（鸿蒙可传第二参 `abilityId`） |
| 页面文案 `"首页"` | `find` + `click` 跳转到该 Tab/页面 |
| `{ appId, abilityId?, settleMs? }` | 对象形式启动 |

启动后 settle：Android / 鸿蒙默认探测 UI 稳定；iOS 默认固定等待（见上文）。
...
| [`nodejs/ios/jd-e2e.mjs`](nodejs/ios/jd-e2e.mjs) | 同 Android 10 步；`goto(bundleId, settleMs)` |
| [`nodejs/ios/jd-mcp-ios.mjs`](nodejs/ios/jd-mcp-ios.mjs) | 同上，全程 MCP |

Python 对照：`python/{web,android,harmony,ios}/jd_e2e.py` ↔ `jd_mcp_*.py`；Web 四场景逻辑已在各脚本内自包含实现。
