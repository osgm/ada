...
- **使用**：`pnpm dlx @ada-mcp/launcher` 或 `@ada-mcp/launcher@0.1.50` 均可；不写 `@x.y.z` 时拉 npm **latest**（推荐生产环境钉版本）。
...
|------|-----------|-------------------|
| **`pnpm dlx @ada-mcp/launcher@0.1.50`**（标准） | launcher 测速 + `.npmrc` → `pnpm dlx` mcp-server | mcp-server preinstall + bootstrap |
| **`npx -y @ada-mcp/launcher@0.1.50`** | 同上 → **`npx -y` mcp-server** | 同上 |
| `pnpm dlx @ada-mcp/mcp-server@0.1.50` | 本机默认源 | 有 preinstall / bootstrap 测速 |
| `npx -y @ada-mcp/mcp-server@0.1.50` | 无 launcher 测速 | 有 preinstall / bootstrap 测速 |
