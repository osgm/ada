/** Sync with packages/install-deps/src/log-locale.ts — run: node scripts/build/sync-log-locale-vendor.mjs */
/** Windows Host（Cursor/JoyCode）常按 GBK 解析 UTF-8 stderr；默认英文，ADA_MCP_LOG_LOCALE=zh 保留中文 */
export function resolveAdaLogLocale() {
...
export function useEnglishAdaLogs() {
...
export function depsLogLine(zh, en) {
    return useEnglishAdaLogs() ? en : zh;
}
/** 包装 onLogLine，保证 install-deps 全链路经本地化 */
export function wrapInstallDepsLogEmitter(onLogLine, scopeHint) {
    if (!onLogLine) {
        return undefined;
    }
    return (line) => {
...
export function localizeAdaLogLine(line) {
