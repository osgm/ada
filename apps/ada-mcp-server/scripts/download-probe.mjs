...
export async function pickFastestProbeUrl(urls, onLogLine) {
    ...
    for (const url of urls) {
        ...
        const mib = (probe.bytesRead / (1024 * 1024)).toFixed(2);
        onLogLine?.(probeLogLine(`[probe]   ${url} -> ${probe.speedKBps.toFixed(0)} KB/s（${mib} MiB / ${probe.durationMs}ms）`, `[probe]   ${url} -> ${probe.speedKBps.toFixed(0)} KB/s (${mib} MiB / ${probe.durationMs}ms)`));
