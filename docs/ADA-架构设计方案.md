...
企业需要一套**跨端、可扩展、业务语义统一**的自动化与控制体系。第一阶段目标是先交付可运行的 `ada-agent`，底层驱动基线收敛为 **Playwright（Web）+ Android/iOS/Harmony 原生驱动插件**，并预留图形交互扩展能力；上层需支持**大模型通过 MCP 调用**，并适应**互联网接入**与**服务端主导控制**场景。
...
- 第一阶段以本地多入口交付为主（`ada-agent` CLI + Web 控制台、`ada-mcp`、`ada-gui`），`ada-control` 延后到第二阶段。
- 驱动基线：`driver-playwright`（Web 默认）+ `driver-android` + `driver-ios` + `driver-harmony`。
- Web 统一按 `playwright` 执行；本地浏览器/用户目录/CDP 由 `driver-playwright` 提供能力。
...
- **入口层**：
  ...
  - Web 控制台（`ada-agent` 内嵌 `web-console` / 打包为 `ada-web.exe`）

统一约束：

- 入口与 MCP/GUI 通过 `agent-core` 或 CLI 调用能力，不在各入口重复实现业务流程。
- 驱动执行由四插件完成：`driver-playwright`、`driver-android`、`driver-ios`、`driver-harmony`（无 Appium/Selenium 中心化 Server）。
- GUI/WEB/MCP 运维动作应优先通过 `agent-core` API 或 `ada-agent core --action=*`。
...
└───────────────────────────┬─────────────────────────────────┘
                            │
┌───────────────────────────▼─────────────────────────────────┐
│  L2 驱动插件：Playwright（Web）/ Android / iOS / Harmony │
└───────────────────────────┬─────────────────────────────────┘
                            │ 桥接（HTTP/gRPC/WS/CLI）
┌───────────────────────────▼─────────────────────────────────┐
...
| 与移动端关系 | 移动应用本体仍由原生 / RN 等构建；Node 侧通过 **Android(UIA2)/iOS(WDA)/Harmony(Hypium+hdc)** 驱动操作 |
...
|-----------|----------------|
| Web（默认） | Playwright |
| Web（可选） | Playwright（本地浏览器 / CDP / 用户目录） |
| Android | ADB + UIAutomator2 adapter（首期） |
| iOS | WDA adapter（首期） |
| HarmonyOS NEXT | hypium-driver + hdc（首期） |
...
| Phase 1 | Web（Playwright）+ 一种移动栈（如 Android UIA2）闭环 |
...
- Web 控制台：`ada-agent web` / 内嵌 `web-console`（可 pkg 为 `ada-web.exe`）
...
| ada-web.exe | 本地 Web 控制台（由 ada-agent 打包） | 单文件可执行程序（pkg） |
| driver plugins | 动态加载模块 | npm 包或独立 worker（按驱动隔离） |

建议在首期优先保证四入口版本统一由同一 `ada-agent` 实现与 `agent-core` 导出构建并发布，避免能力漂移。
...
| `plugins/driver-playwright` / `driver-android` / `driver-ios` / `driver-harmony` | 各平台驱动插件（Web / Android / iOS / Harmony） | 只依赖 plugin-sdk + 对应桥接库 |
...
| Android（ADB/UIA2） | 会话与命令语义、元素生命周期 | 自研 adapter + 命令转换层，不透传底层原始模型到业务层 |
...
| M3 | `driver-android`/`driver-ios`/`driver-harmony` 完成，三平台可用 |
...
- **Android + iOS + HarmonyOS NEXT**：`driver-android` / `driver-ios` / `driver-harmony`
...
#### 13.9.4 与 `driver-playwright` / 移动驱动插件的协同

- `driver-playwright`：优先 DOM/ARIA 语义定位；图像动作仅用于 Canvas/WebGL/远程桌面类页面兜底。
- 移动驱动插件：优先控件树定位（Android UIA2 / iOS WDA 链路）；图像模式用于复杂自绘界面与无障碍信息缺失场景。
...
| 6 | `driver-android` + `driver-ios` | Android+iOS 统一桥接、会话管理、动作映射 | 移动驱动插件 |
...
| W7-W8 | 移动闭环 | 实现 `driver-android`/`driver-ios`（基础动作） | Android/iOS 冒烟各 >= 20 条通过 |
...
#### E. `plugins/driver-android` 与 `plugins/driver-ios`
...
- 关键路径：`contracts -> core-kernel -> plugin-sdk/plugin-host -> driver-playwright -> driver-android/ios/harmony -> 打包发布`
...
1. `driver-playwright` 与移动驱动 conformance 测试通过。  
...
6. 首启自动依赖安装（Playwright/移动驱动）在目标网络环境可用或已提供手动安装替代流程。  
...
## 17. 当前代码架构（文件级，2026-05）

本节补充“可直接对照仓库”的代码架构视图，便于研发、排障与新成员快速定位。

### 17.1 Monorepo 分层映射

| 层 | 目录 | 关键职责 |
|----|------|---------|
| 入口层 | `apps/ada-agent`、`apps/ada-mcp-server`、`apps/ada-mcp-launcher`、`apps/ada-gui` | 对外交互、协议接入、启动流程 |
| 领域与编排层 | `apps/ada-agent`（CLI/队列/runtime）、`packages/agent-core`（导出）、`packages/core-kernel` | 能力编排、运行策略、health/doctor/setup/start/run |
| 依赖安装层 | `packages/install-deps`、`packages/runtime-probe` | npm/浏览器/hdc 安装、镜像测速、移动运行时探针 |
| 协议与传输层 | `packages/contracts`、`packages/transport-http`、`packages/transport-stream`、`packages/driver-rpc` | 统一协议、请求响应、流式通信 |
| 插件框架层 | `packages/plugin-host`、`packages/plugin-sdk` | 驱动插件注册、加载与调用契约 |
| 驱动执行层 | `plugins/driver-playwright`、`plugins/driver-android`、`plugins/driver-ios`、`plugins/driver-harmony` | 平台动作落地执行 |
| 基础能力层 | `packages/core-runtime`、`packages/download-probe` | 工作区定位、下载测速与依赖安装镜像探测 |

### 17.2 主调用链（CLI）

```text
apps/ada-agent/src/main.ts
  -> packages/agent-core（health/doctor/install-deps/run 导出）
  -> packages/install-deps（npm/浏览器/hdc 安装、InstallSummary）
  -> packages/runtime-probe（adb、xcrun/WDA 探针）
  -> runtime.ts / queue-runner.ts / plugin-registry.ts
  -> plugins/driver-{playwright,android,ios,harmony}
```

说明：

- `main.ts` 是 `@ada/agent` 的统一入口，负责命令分发（`run/start/install-deps/doctor/...`）。
- `packages/install-deps` 是依赖与工具下载中枢（Playwright/Harmony npm、浏览器、hdc）。
- MCP / GUI / Web 通过 `agent-core` 或 `@ada/install-deps` 调用同一安装实现，避免入口重复。

### 17.3 主调用链（MCP）

```text
apps/ada-mcp-launcher/launcher.mjs
  -> 选择 registry / 版本兜底
  -> 拉起 @ada-mcp/mcp-server
      -> apps/ada-mcp-server/src/cli.ts
      -> apps/ada-mcp-server/src/main.ts (tool 定义与调用路由)
      -> 复用 @ada/agent 能力执行
```

说明：

- `launcher.mjs` 是“运行时分发器”，负责版本与安装策略，不承载业务执行逻辑。
- `mcp-server/src/main.ts` 是 MCP 工具语义入口；最终执行仍落到 `@ada/agent` 与驱动插件体系。
- `mcp-server` 与 `launcher` 必须同号发布（仓库已有 `scripts/release/check-mcp-publish-versions.mjs` 门禁）。

### 17.4 关键模块边界（当前实践）

| 模块 | 应承担 | 不应承担 |
|------|--------|----------|
| `packages/install-deps` | 依赖安装、工具下载、平台前置检查、`InstallSummary` | 业务动作编排与插件动作语义 |
| `packages/install-deps/src/tools-paths.ts` | 工具路径候选解析与环境注入 | 下载策略决策 |
| `apps/ada-mcp-launcher/launcher.mjs` | 拉起策略、源探测、版本兜底 | 工具执行与平台行为逻辑 |
| `apps/ada-mcp-server/src/main.ts` | MCP tool schema 与调用路由 | 平台底层驱动细节实现 |
| `plugins/driver-playwright` 等四平台插件 | 平台执行适配（action/locator/assertion） | 入口层配置读写与全局安装流程 |

### 17.5 当前热点代码（建议优先读）

1. `packages/install-deps`：依赖安装全链路（含 Harmony hdc/zip/libusb 逻辑）。
2. `packages/install-deps/src/tools-paths.ts`：`ADA_TOOLS_DIR`、`HDC_HOME`、PATH 注入与工具目录探测。
3. `apps/ada-mcp-launcher/launcher.mjs`：版本检测、最低版本约束、启动参数透传。
4. `apps/ada-mcp-server/src/main.ts`：MCP 工具面定义、调用入口、结果封装。
5. `packages/plugin-host` + 四平台驱动插件（playwright / android / ios / harmony）：插件化执行主链路。

### 17.6 排障定位建议（代码架构视角）

- “npm 已更新但 Cursor 仍报错”：先看 `launcher` 版本与 Host 配置是否仍钉旧版本。
- “Harmony 提示 hdc 不可用”：先看 `packages/install-deps` 内 `tools-paths.ts` 决定的最终工具目录，再看安装流程是否补齐 DLL。
- “MCP 可用但 CLI 不一致”：对比是否走了相同 `@ada/agent` 配置与 `cwd`/env。
