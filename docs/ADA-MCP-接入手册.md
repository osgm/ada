...
```json
{
  "mcpServers": {
    "ada-mcp": {
      "command": "pnpm",
      "args": ["dlx", "@ada-mcp/launcher@0.1.84"]
    }
  }
}
```
...
```bash
pnpm dlx @ada-mcp/launcher@0.1.84
# npx 等价（Windows 建议仍用 launcher，勿单独 npx mcp-server）
npx -y @ada-mcp/launcher@0.1.84
```

| 包 | 作用 |
|----|------|
| `@ada-mcp/launcher` | registry 测速 → `pnpm dlx` / `npx` → mcp-server（**零 npm 依赖**） |
| `@ada-mcp/mcp-server` | **26** 个 MCP 工具（默认隐藏 T3 时 **23**）、依赖安装、驱动插件 |
...
Cursor/Agent 高级自动化推荐 `ADA_MCP_HIDE_ADVANCED=0`（暴露 `ada_invoke` / `ada_risk_policy`），可选 `ADA_MCP_DESC_MODE=advanced` 加长 L3 工具说明。脚本侧 `connectMcp()` 默认仍为 `ADA_MCP_HIDE_ADVANCED=1`（仅 T1 语义工具）；需要驱动级 API 时在 env 中设为 `0`。

### 1.2 版本与 registry

| 写法 | 行为 |
|------|------|
| `@ada-mcp/launcher@0.1.84` | 钉版本（生产推荐，与 mcp-server 同号） |
...
| `playwright` / `mobile` / `android` / `ios` / `harmony` / `drivers` / `all` | 见名；可逗号组合。`all` = Playwright（npm+浏览器）+ 移动驱动（Android/UIA2、iOS/WDA、Harmony/hypium+hdc）；`drivers` = 仅移动工具链/驱动环境检查（不装 Appium 等中心化 Server） |
| `none` / `skip` 或 `ADA_MCP_SKIP_INSTALL_DEPS=1` | 跳过 |

Host 示例：`"args": ["dlx", "@ada-mcp/launcher@0.1.84", "--install-deps=playwright,mobile"]`

工作区：`.ada-mcp-playwright-host` 写在 **Host 项目目录**（`INIT_CWD`），勿写 monorepo 源码路径。

### 1.4 MCP Host 启动超时（`-32001 Request timed out`）

部分 IDE（如 JoyCode / Cursor）对 MCP **stdio 握手约 60s**。`npx -y @ada-mcp/launcher` 冷启动时，若再执行 **串行** registry + Playwright preinstall 测速，最坏可超过 60s。

**推荐（0.1.75+，当前 monorepo 0.1.84）**：
...
```json
{
  "mcpServers": {
    "ada-mcp": {
      "command": "npx",
      "args": ["-y", "@ada-mcp/launcher@0.1.84"],
      "env": {
        "ADA_MCP_REGISTRY": "https://registry.npmmirror.com"
      }
    }
  }
}
...
| 变量 | 作用 |
|------|------|
| `ADA_MCP_FAST_START` / `ADA_MCP_QUICK_START` | **默认已开启**（launcher 子进程亦注入）：3 镜像并行测速 ≤5s、跳过 preinstall、版本 pin 到 launcher 同号 |
| `ADA_MCP_SLOW_START` | `1` 关闭快速握手，恢复完整 registry 测速与 latest 查询 |
| `ADA_MCP_ALLOW_BOOTSTRAP_ALL` | `1` 时保留 args 中的 `--install-deps=all`（默认降为 `playwright` 以优先 MCP 握手） |
| `ADA_MCP_REGISTRY` | 强制镜像，**跳过** registry 测速 |
| `ADA_MCP_SKIP_PREINSTALL_PROBE` | 仅跳过 preinstall 测速 |
| `ADA_PROBE_DOWNLOAD_TIMEOUT_MS` | 单次下载样本超时（ms） |
| `ADA_MCP_REGISTRY_PROBE_MAX_MS` | 并行 registry 测速整轮上限（ms） |
| `ADA_MCP_REGISTRY_PROBE_TTL_MS` | registry 测速缓存 TTL（默认 3600000） |
| `ADA_MCP_FORCE_PLAYWRIGHT_PROBE` | `1` 强制 bootstrap 重新测 Playwright CDN |
| `ADA_MCP_FORCE_REGISTRY_PROBE` | `1` 强制 bootstrap 重新测 npm registry |
| `ADA_MCP_GLOBAL_INSTALL_LOCK_WAIT_MS` | 并发 launcher 等待全局安装锁（默认 120000） |
| `ADA_MCP_LOG_LEVEL` | 默认 **`info`**（输出 registry/Playwright 测速与 `[deps]` 安装日志）；静默设 `error` 或 `ADA_MCP_QUIET=1` |
| `ADA_MCP_LOG_INFO_STDERR` | `1` 强制 info 写 stderr；默认 info 写 **stdout**（launcher）或 MCP **logging**（mcp-server 握手后） |
| `ADA_MCP_STRUCTURED_PROGRESS` | 默认开启；`0` 关闭结构化安装进度（MCP logging JSON + stderr `[ADA-MCP][progress]`） |
| `ADA_MCP_TOOL_VISIBILITY` | `ADA_MCP_HIDE_ADVANCED` 的别名；`hide` / `1` 隐藏 T3（`ada_invoke` / `ada_risk_policy`） |
| `ADA_MCP_VERBOSE_RESULT` | `1` 时 MCP 工具返回完整 `CommandResult`（默认 slim 截断大字段） |
| `ADA_MCP_SLIM_RESULT` | `0` 显式关闭 slim（等同 verbose） |
| `ADA_MCP_PREFLIGHT_CACHE_MS` | 移动端 runtime preflight 成功缓存 TTL（默认 60000） |
| `ADA_MCP_PROBE_CACHE_MS` | 移动端 probe 结果缓存 TTL（默认 45000） |
**勿**在 `args` 中加 `--install-deps=all`（会拉长首启）；依赖就绪后用 `ada_install_deps`。终端可预热：

```bash
npx -y @ada-mcp/launcher@<ver> --skip-install-deps
# MCP 连上后：ada_install_deps only=playwright
```
...
| 字段 | 含义 |
|------|------|
| `mcpBootstrap` | 后台安装进度（`active` / `phase` / `scopes`，握手后即有 planned scopes） |
| `readiness` | **可连 MCP** vs **可跑 Web/移动** 提示（`hint`、`webAutomationReady` 等） |
| `installProgress` | 最新结构化进度事件（`kind=ada.install.progress`，含 `phase` / `percent` / `message`） |
| `dependencies.packagesReady` | npm 包/state 标记（≠ 设备就绪） |
| `dependencies.runtimeReady` | 浏览器启动 / hdc 等探测 |

**`packagesReady.mobile` 为 true 不表示 adb/设备已就绪**，移动自动化仍需 `ada_devices` + `ada_install_deps`。

直接 `pnpm dlx @ada-mcp/mcp-server`（不经 launcher）时，进程启动会 **standalone seed** 默认 registry + Playwright CDN 到 `~/.ada/deps-install-state.json`（可用 `ADA_MCP_REGISTRY` 覆盖）。launcher 仅在项目目录（含 `package.json` / `.git` 等）写入 `.npmrc`；`ADA_MCP_WRITE_PROJECT_NPMRC=1` 可强制写入。

**0.1.64+ 首启优化**：

| 现象 | 处理 |
|------|------|
| 外层 npx、内层 pnpm 不一致 | `auto` 已按 Host 识别；日志见 `outer=npx runner=npx` |
| registry 测速重复 / 慢 | launcher 文件锁 + **1h** 缓存（`ADA_MCP_REGISTRY_PROBE_TTL_MS`）；launcher 版本变更自动失效；子进程复用 `deps-install-state.json`（registry + Playwright CDN），bootstrap 默认跳过二次测速 |
| Host 重连多次 launcher | 并发 launcher 跳过重复测速（`launcher-inflight.json`） |
| `releasing sessions (stdin-close)` 首启断连 | 装依赖期间 **不释放** 会话（默认 debounce 300s）；`ADA_MCP_STDIN_BOOTSTRAP_DEBOUNCE_MS` 可调 |
| Windows 中文乱码 | 启动时 `chcp 65001`；关键 `[deps]` 日志默认英文，设 `ADA_MCP_LOG_LOCALE=zh` 恢复中文 |

### 1.5 复用本机已安装的 Playwright 浏览器

未设置 `PLAYWRIGHT_BROWSERS_PATH` 时，**0.1.62+** `@ada/install-deps` 会自动扫描常见缓存目录并复用（日志：`复用已有 Playwright 浏览器目录`）：

| 系统 | 扫描路径（节选） |
|------|------------------|
| Windows | `%LOCALAPPDATA%\ms-playwright`、`%USERPROFILE%\AppData\Local\ms-playwright` |
| macOS | `~/Library/Caches/ms-playwright` |
| Linux | `$XDG_CACHE_HOME/ms-playwright` 或 `~/.cache/ms-playwright` |
| 共用 | `~/.ada/playwright-browsers`（若已有浏览器则优先于空目录） |

关闭自动扫描：`ADA_PLAYWRIGHT_AUTO_DISCOVER=0`（回退仅使用 `~/.ada/playwright-browsers`）。手动指定仍用 `PLAYWRIGHT_BROWSERS_PATH`。
...
`ada-mcp-server` 暴露 **25** 个工具（`ada_` + 蛇形命名）。`ADA_MCP_HIDE_ADVANCED=1`（或 `ADA_MCP_TOOL_VISIBILITY=hide`）时隐藏 **T3** 共 2 个（`ada_invoke`、`ada_risk_policy`），ListTools 返回 **23** 个。

| # | 工具 | 分类 | 何时选用（AI 路由提示） |
|---|------|------|------------------------|
| 1 | `ada_health` | 观测 | 会话首选：快速探活、依赖是否就绪 |
| 2 | `ada_install_deps` | 配置 | 缺 Playwright/移动依赖时安装（`only` 含 `drivers`） |
| 3 | `ada_devices` | 观测-Mobile | **移动前置**：扫描/列出 Android/iOS/Harmony 设备与推荐 capabilities |
| 4 | `ada_diagnostics` | 观测 | 健康异常时深度诊断（Node/驱动/路径） |
| 5 | `ada_config` | 配置 | 读取生效配置与路径 |
| 6 | `ada_plugins` | 观测 | 确认已加载 playwright/android/ios/harmony |
...
| 14 | `ada_assertions` | 数据-Web | 断言可见/文案/URL |
| 15 | `ada_mobile_action` | 执行-Mobile | **首选**：Android/iOS/Harmony 点击/滑动/启停 App |
| 16 | `ada_mobile_recipe` | 执行-Mobile | L2 recipe：`dump_ui` / `viewTree`（经 extract）/ `tap_path` / `tap_search` / `fill_search` |
| 17 | `ada_mobile_dismiss_popups` | 执行-Mobile | 关闭移动端弹窗（找不到不报错） |
| 18 | `ada_mobile_extract` | 数据-Mobile | 观察 `type=viewTree`（扁平可点控件）；或 `text` / `pageSource` |
| 19 | `ada_mobile_assertions` | 数据-Mobile | 移动端可见/文案断言 |
| 20 | `ada_batch_actions` | 编排 | 同会话多步串联（无文件） |
| 21 | `ada_run_task_file` | 编排 | 运行 `.tasks.json` 场景文件 |
| 22 | `ada_sessions` | 会话 | 列出活跃 sessionId |
| 23 | `ada_close_session` | 会话 | 关闭单个会话 |
| 24 | `ada_close_all_sessions` | 会话 | 关闭全部会话（收尾/重载 MCP） |
| 25 | `ada_risk_policy` | 安全（**T3**） | 查看/维护高风险命令白名单 |

> MCP `ListTools` 返回的 **description** 含 `[Lx-…]` 层级前缀、`USE WHEN`、`KEY ARGS` 等结构化提示；源码见 `apps/ada-mcp-server/src/mcp-tool-definitions.ts`、`mcp-tool-tiers.ts`。
...
底层 API    → ada_invoke（T3，需 riskApproved）
...
1. `ada_extract` + `mode=viewTree`：默认 `payload.detail=controls`（扁平可点列表）；需层级时用 `detail=tree` 或 `detail=full`（无重复 `regions`）
...
**Mobile 遍历（平台观察 + Agent 策略）**：

1. `ada_mobile_extract` + `type=viewTree`：返回扁平可点控件（含 `path` 标签段）
2. Agent 根据控件决定下一步；路径点击用 `ada_mobile_recipe` + `action=tap_path` + `path`（每段之间会 re-dump UI）
3. 也可用 `ada_mobile_action` + locator；搜索框场景用 `tap_search` / `fill_search`
4. 连续重复 click/swipe/tap_path 会触发 **动作熔断**，阈值同 Web（`ADA_WEB_ACTION_LEDGER_*`）
...
**发布前验证**（维护者，自 monorepo 根目录）：

```bash
npm run mcp:check:versions    # launcher 与 mcp-server 版本对齐
...
Android **adb method** invoke（无需 UiAutomator2 Server）。`mode` / `target` / `method` 放**顶层**（与 `ada_invoke` inputSchema 一致）：
...
| `ADA_ANDROID_DEVICE_SN` | 默认 Android 设备 serial（多设备时与 `capabilities.udid` 二选一） |
| `ADA_ANDROID_UIA2_LOCAL_PORT` | adb forward 本机端口（未设 `ADA_ANDROID_UIA2_SERVER_URL` 时用于拼 URL） |
| `ADA_ANDROID_UIA2_DEVICE_PORT` | 设备端 UIA2 端口，默认 6790 |
...
| `ADA_WEB_PAGE_PROBE_TTL_MS` | Web 页探活缓存毫秒数（`ensureWebPageReady`），默认 `2000`；设 `0` 禁用 |
...
| `ADA_IOS_LOCAL_HOST` | WDA/iproxy 本机 host，默认 `localhost` |
| `ADA_IOS_IPROXY_READY_MS` | iproxy 启动后等待本机端口就绪超时（默认 10000） |
...
| `ada_install_deps` | `{ "only": "playwright" }` 或 `{ "only": "drivers" }`（仅移动工具链检查） |
...
Agent **transport**（`apps/ada-agent`）走 HTTP/WebSocket 时映射为 `ada_web_action` / `ada_mobile_action` / `ada_mobile_recipe`。
...
| `POST /mcp` | Streamable HTTP MCP（完整工具集 + recoveryPlan；需 API Key） |

---

## 5. 命令超时（防挂起）

内核对所有 `ada_mobile_action` / `ada_web_action` 命令施加**墙钟超时**（与 `payload.timeoutMs` 的「等待/定位」语义分离）：
...
| `ADA_MCP_INSTALL_DEPS` / `ADA_MCP_SKIP_INSTALL_DEPS` | 启动时装依赖（`only` 含 `drivers`） |
| `ADA_MCP_HIDE_ADVANCED` / `ADA_MCP_TOOL_VISIBILITY` | 隐藏 T3 工具（`hide` / `1`） |
| `ADA_MCP_VERBOSE_RESULT` / `ADA_MCP_SLIM_RESULT` | MCP 响应 slim / verbose 控制 |
| `ADA_ANDROID_DEVICE_SN` | 默认 Android 设备 serial |
...
内置 registry 候选（仅 3 个，并行测速 ≤5s）：**npmmirror（阿里）** → **华为云** → **npmjs 官网**。

**直连 mcp-server**（无 launcher 测速）：

```json
{ "command": "npx", "args": ["-y", "@ada-mcp/mcp-server@0.1.84"] }
```
...
| `'mcp-server' 不是内部或外部命令`（Win） | 勿 `npx @ada-mcp/mcp-server`；用 **`pnpm dlx @ada-mcp/launcher@0.1.84`** |
| `spawn EINVAL`（Win，`npm.cmd` / npx 回退） | 升级到 **`@ada-mcp/launcher@0.1.75+`**；或改用 `pnpm dlx` |
...
| zod / `ERR_PACKAGE_PATH_NOT_EXPORTED` | 清 dlx 缓存，钉 `@ada-mcp/launcher@0.1.84` |
| dlx 仍是旧版 | 钉版本或清 `pnpm-cache/dlx` |
| MCP `-32001 Request timed out`（约 60s） | 设 `ADA_MCP_REGISTRY` 固定镜像；**勿**在 args 加 `--install-deps=all`（launcher 默认降为 playwright）；需完整测速时 `ADA_MCP_SLOW_START=1`；见 §1.4 |
| Mac `mkdir '/tools' ENOENT`（首次 `install-deps=all`） | MCP Host 未设 `cwd` 时进程可能在 `/`；已修复为改用 `~/.ada/tools`；或 Host 设 `cwd` 为项目目录、`ADA_TOOLS_DIR` 指向已有 tools |
...
**版本约定**：每次发布 `@ada-mcp/mcp-server` 与 `@ada-mcp/launcher` 的 `package.json` 的 `version` **必须相同**；先发布 mcp-server，再发布 launcher。发布前执行：

```bash
npm run mcp:check:versions && npm run test:mcp:unit && npm run mcp:preflight
```
