...
**推荐版本（2026-06）**：`@ada-mcp/launcher@0.1.75` 与 `@ada-mcp/mcp-server@0.1.75` **同号发布**（示例：`pnpm dlx @ada-mcp/launcher@0.1.75`）。**勿用 0.1.73 / 0.1.74**（已知启动缺陷，已 deprecate）。
...
| 写法 | 行为 |
|------|------|
| `@ada-mcp/launcher@0.1.75` | 钉版本（生产推荐） |
...
Host 示例：`"args": ["dlx", "@ada-mcp/launcher@0.1.75", "--install-deps=playwright,mobile"]`
...
```json
{ "command": "npx", "args": ["-y", "@ada-mcp/mcp-server@0.1.75"] }
```
...
| `'mcp-server' 不是内部或外部命令`（Win） | 勿 `npx @ada-mcp/mcp-server`；用 **`pnpm dlx @ada-mcp/launcher@0.1.75`** |
...
| zod / `ERR_PACKAGE_PATH_NOT_EXPORTED` | 清 dlx 缓存，钉 `@ada-mcp/launcher@0.1.75` |
