...
| 场景 | 入口 |
|------|------|
| **对外用户（标准）** | `pnpm dlx @ada-mcp/launcher@0.1.24`（npx：`npx -y @ada-mcp/launcher@0.1.24`）→ `@ada-mcp/mcp-server@0.1.24` |
...
通过 **`@ada-mcp/launcher`** 拉起 **`@ada-mcp/mcp-server@0.1.24`**（launcher 负责拉包前 registry 测速；mcp-server 负责 MCP 工具、Playwright 浏览器安装与依赖自检）。
...
| 包 | npm latest | 作用 |
|----|------------|------|
| `@ada-mcp/launcher` | **0.1.24** | 与 mcp-server 同版本；测速选镜像；config hint 自动对齐版本 |
| `@ada-mcp/mcp-server` | **0.1.24** | Appium3 驱动名；已安装仅 warn；xcuitest 仅 macOS；安装日志与 CDN |

> **0.1.18 要点**：mcp-server `install-deps` 前自动卸载冲突 playwright/zod；launcher **0.1.18** 修复 Windows `spawn EINVAL`（经 `cmd.exe /c` 执行 pnpm/npx）。  
> **0.1.24 要点**：文档与示例统一；launcher `MIN_MCP_SERVER_VERSION=0.1.24`；geckodriver 镜像说明与安装日志修复。

命令行等价：

```bash
pnpm dlx @ada-mcp/launcher@0.1.24
```

**npx 等价**（`0.1.7+`：launcher 测速逻辑与 pnpm 相同，内层用 **`npx -y` 拉 mcp-server**，无需再装 pnpm）：

```bash
npx -y @ada-mcp/launcher@0.1.24
```

| 场景 | pnpm | npx |
|------|------|-----|
| 标准启动 | `pnpm dlx @ada-mcp/launcher@0.1.24` | `npx -y @ada-mcp/launcher@0.1.24` |
| 跳过自动装依赖 | `…@0.1.12 --skip-install-deps` | `npx -y @ada-mcp/launcher@0.1.24 --skip-install-deps` |
| Cursor `command` | `pnpm` | `npx` |
| Cursor `args` | `["dlx", "@ada-mcp/launcher@0.1.24"]` | `["-y", "@ada-mcp/launcher@0.1.24"]` |
| launcher 内层拉起 mcp-server | `pnpm dlx @ada-mcp/mcp-server@0.1.24` | `npx -y @ada-mcp/mcp-server@0.1.24` |
...
> 说明：进程会挂起等待 stdio，属正常现象。**推荐** `pnpm dlx @ada-mcp/launcher`（自动 latest）或钉 `@0.1.24`。

#### 3.9.0 版本号：安装时要不要写 `@0.1.24`？
...
| 写法 | 行为 |
|------|------|
| `@ada-mcp/launcher@0.1.24` | 固定 launcher **0.1.24**（可复现）；不写版本则拉 registry **latest** |
...
直连 mcp-server 时同理：`@ada-mcp/mcp-server@0.1.24` 为钉版本；`@ada-mcp/mcp-server` 为 latest。
...
```json
"args": ["dlx", "@ada-mcp/launcher@0.1.24", "--install-deps=playwright,selenium"]
```
...
```json
"args": ["dlx", "@ada-mcp/launcher@0.1.24", "--install-deps=selenium"]
```
...
使用 **§3.9 标准配置**（`@ada-mcp/launcher@0.1.24`）时，一般**无需**再配 `npm_config_registry` 等镜像环境变量（pnpm / npx 均会写入 `npm_config_registry` 与项目 `.npmrc`）。

依赖下载分 **三个阶段**：

| 阶段 | 行为 |
|------|------|
| **A. launcher 拉包** | 测速选最快 registry，经 `npm_config_registry` + 项目 `.npmrc`；**pnpm dlx 或 npx -y** 安装 `@ada-mcp/mcp-server@0.1.24`（`0.1.7+` 与外层 runner 一致） |
...
```json
"args": ["dlx", "@ada-mcp/mcp-server@0.1.24"]
```
...
```json
{
  "mcpServers": {
    "ada-mcp": {
      "command": "npx",
      "args": ["-y", "@ada-mcp/mcp-server@0.1.24"]
    }
  }
}
```
...
- `spawn EINVAL`（Windows，`commandAvailable` / launcher 启动失败）：升级到 **`@ada-mcp/launcher@0.1.18+`**（推荐 **0.1.24**）；勿在 `C:\Windows\System32` 下运行。`pnpm dlx` 起 launcher 时会直接选用 pnpm，不再探测 `pnpm.cmd`。
- **`pnpm dlx @ada-mcp/launcher@latest` 仍是旧版**（堆栈路径含 `@ada-mcp+launcher@0.1.xx`）：常见原因是 **pnpm dlx 缓存**（`@latest` 作缓存键，默认约 24 小时内复用）或本机 **minimumReleaseAge** 限制 immature 版本。立刻可用：`pnpm dlx @ada-mcp/launcher@0.1.24`；或删除 `pnpm-cache/dlx` 下对应 hash 目录后再跑 `@latest`。
...
- `playwright install` / `NoSuchKey` / `404`（npmmirror）：升级到 **`mcp-server@0.1.24`**；删除 `.ada-mcp-playwright-host` 后设置 `PLAYWRIGHT_DOWNLOAD_HOST=https://cdn.playwright.dev` 重试。
- `chromium_headless_shell` 不存在、仅装了系统 Chrome：升级到 **`mcp-server@0.1.8+`**（默认改装 `chromium`）；勿长期停留在 `0.1.6`。
- `ERR_PACKAGE_PATH_NOT_EXPORTED` / `zod` / `./v3`：日志里若出现 **`mcp-server@0.1.10`**，多为 **pnpm 缓存了旧 launcher** 或 `ADA_MCP_SERVER_VERSION=0.1.10`。请 **`pnpm dlx @ada-mcp/launcher@latest`** 或 **`@0.1.24+`**（内层从 registry 读 mcp-server `latest`）。检查：`echo %ADA_MCP_SERVER_VERSION%`。
