...
| 场景 | 入口 |
|------|------|
| **对外用户（标准）** | `pnpm dlx @ada-mcp/launcher@0.1.27`（npx：`npx -y @ada-mcp/launcher@0.1.27`）→ `@ada-mcp/mcp-server@0.1.27` |
...
通过 **`@ada-mcp/launcher`** 拉起 **`@ada-mcp/mcp-server@0.1.27`**（launcher 负责拉包前 registry 测速；mcp-server 负责 MCP 工具、Playwright 浏览器安装与依赖自检）。
...
| 包 | npm latest | 作用 |
|----|------------|------|
| `@ada-mcp/launcher` | **0.1.27** | 与 mcp-server 同版本；测速选镜像；config hint 自动对齐版本 |
| `@ada-mcp/mcp-server` | **0.1.27** | Appium3 驱动名；已安装仅 warn；xcuitest 仅 macOS；安装日志与 CDN |

> **0.1.18 要点**：mcp-server `install-deps` 前自动卸载冲突 playwright/zod；launcher **0.1.18** 修复 Windows `spawn EINVAL`（经 `cmd.exe /c` 执行 pnpm/npx）。  
> **0.1.24 要点**：文档与示例统一；geckodriver 镜像说明与安装日志修复。  
> **0.1.26 要点**：修复 `pnpm dlx` 后插件目录解析（`ADA_MCP_SERVER_ENTRY`、`resolvePackagePluginDir`）；消除 `WEB_ENGINE_UNKNOWN: playwright`；`executor` 单例 PluginHost。  
> **0.1.27 要点（正式版）**：与 0.1.26 能力对齐并统一文档/示例版本；`MIN_MCP_SERVER_VERSION=0.1.27`；插件重复注册防护；推荐生产环境钉 `@0.1.27`。
...
| 场景 | pnpm | npx |
|------|------|-----|
| 标准启动 | `pnpm dlx @ada-mcp/launcher@0.1.27` | `npx -y @ada-mcp/launcher@0.1.27` |
| 跳过自动装依赖 | `…@0.1.12 --skip-install-deps` | `npx -y @ada-mcp/launcher@0.1.27 --skip-install-deps` |
| Cursor `command` | `pnpm` | `npx` |
| Cursor `args` | `["dlx", "@ada-mcp/launcher@0.1.27"]` | `["-y", "@ada-mcp/launcher@0.1.27"]` |
| launcher 内层拉起 mcp-server | `pnpm dlx @ada-mcp/mcp-server@0.1.27` | `npx -y @ada-mcp/mcp-server@0.1.27` |
...
> 说明：进程会挂起等待 stdio，属正常现象。**推荐** `pnpm dlx @ada-mcp/launcher`（自动 latest）或钉 `@0.1.27`。

#### 3.9.0 版本号：安装时要不要写 `@0.1.27`？
...
| 写法 | 行为 |
|------|------|
| `@ada-mcp/launcher@0.1.27` | 固定 launcher **0.1.27**（可复现）；不写版本则拉 registry **latest** |
...
直连 mcp-server 时同理：`@ada-mcp/mcp-server@0.1.27` 为钉版本；`@ada-mcp/mcp-server` 为 latest。
...
```json
"args": ["dlx", "@ada-mcp/launcher@0.1.27", "--install-deps=playwright,selenium"]
```
...
```json
"args": ["dlx", "@ada-mcp/launcher@0.1.27", "--install-deps=selenium"]
```
...
Cursor / MCP 配置里**不要**把 `ADA_PLUGIN_DIR`、`dirver/` 等写成 ada 源码仓库路径（如 `D:/WORKSPACE/PLAN/ada/...`）。运行时按下面规则自动解析：
...
使用 **§3.9 标准配置**（`@ada-mcp/launcher@0.1.27`）时，一般**无需**再配 `npm_config_registry` 等镜像环境变量（pnpm / npx 均会写入 `npm_config_registry` 与项目 `.npmrc`）。

依赖下载分 **三个阶段**：

| 阶段 | 行为 |
|------|------|
| **A. launcher 拉包** | 测速选最快 registry，经 `npm_config_registry` + 项目 `.npmrc`；**pnpm dlx 或 npx -y** 安装 `@ada-mcp/mcp-server@0.1.27`（`0.1.7+` 与外层 runner 一致） |
...
```json
"args": ["dlx", "@ada-mcp/mcp-server@0.1.27"]
```
...
```json
{
  "mcpServers": {
    "ada-mcp": {
      "command": "npx",
      "args": ["-y", "@ada-mcp/mcp-server@0.1.27"]
    }
  }
}
```
...
- `spawn EINVAL`（Windows，`commandAvailable` / launcher 启动失败）：升级到 **`@ada-mcp/launcher@0.1.18+`**（推荐 **0.1.27**）；勿在 `C:\Windows\System32` 下运行。`pnpm dlx` 起 launcher 时会直接选用 pnpm，不再探测 `pnpm.cmd`。
- **`pnpm dlx @ada-mcp/launcher@latest` 仍是旧版**（堆栈路径含 `@ada-mcp+launcher@0.1.xx`）：常见原因是 **pnpm dlx 缓存**（`@latest` 作缓存键，默认约 24 小时内复用）或本机 **minimumReleaseAge** 限制 immature 版本。立刻可用：`pnpm dlx @ada-mcp/launcher@0.1.27`；或删除 `pnpm-cache/dlx` 下对应 hash 目录后再跑 `@latest`。
...
- `Could not resolve driver` / 安装 `appium-uiautomator2-driver` 失败：多为 **Appium 3** 下误用 npm 包名。升级到 **`mcp-server@0.1.20+`**（内层执行 `appium driver install uiautomator2`），或本机手动：`cd %APPIUM_HOME%` 所在项目后 `npx appium driver install uiautomator2`。
...
- `playwright install` / `NoSuchKey` / `404`（npmmirror）：升级到 **`mcp-server@0.1.27`**；删除 `.ada-mcp-playwright-host` 后设置 `PLAYWRIGHT_DOWNLOAD_HOST=https://cdn.playwright.dev` 重试。
- `chromium_headless_shell` 不存在、仅装了系统 Chrome：升级到 **`mcp-server@0.1.8+`**（默认改装 `chromium`）；勿长期停留在 `0.1.6`。
- `ERR_PACKAGE_PATH_NOT_EXPORTED` / `zod` / `./v3`：日志里若出现 **`mcp-server@0.1.10`**，多为 **pnpm 缓存了旧 launcher** 或 `ADA_MCP_SERVER_VERSION=0.1.10`。请 **`pnpm dlx @ada-mcp/launcher@latest`** 或 **`@0.1.27+`**（内层从 registry 读 mcp-server `latest`）。检查：`echo %ADA_MCP_SERVER_VERSION%`。
- `dlx` 很慢但 `install-deps` 正常：阶段 A 未走 ADA 探测，请用 **launcher** 或在 `env` 中设置 `npm_config_registry`（见 §3.9.3）。
